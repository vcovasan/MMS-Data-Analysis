function shockVelocity3(i1,i2,i4, N)
...
%rxyz(i1,:)
r1x=r1x(1:min([length(r1x), length(r2x)]));
r2x=r2x(1:min([length(r1x), length(r2x)]));
%r4x=r4x(1:min([length(r1x), length(r2x)]));

r1y=r1y(1:min([length(r1y), length(r2y)]));
r2y=r2y(1:min([length(r1y), length(r2y)]));
%r4y=r4y(1:min([length(r1y), length(r2y)]));


r1z=r1z(1:min([length(r1z), length(r2z)]));
r2z=r2z(1:min([length(r1z), length(r2z)]));
...
vx=vx12;% see the shape of the tetrahedron to check
vy=vy12;%  which pairs are reliable and which not
vz=vz12;% if too close, discard 
...
minLength=min([length(r1),length(r2)]);% normalise wrt the shortest dataset 
r1=r1(1:minLength);
r2=r2(1:minLength);
%r3=r3(1:minLength);
%r4=r4(1:minLength);
