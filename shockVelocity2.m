...
function [V] = shockVelocity2(i1,i3,i4, N)
...
minLengthrx=min([length(r1x) length(r3x) length(r4x)]);
minLengthry=min([length(r1y) length(r3y) length(r4y)]);
minLengthrz=min([length(r1z) length(r3z) length(r4z)]);

r1x=r1x(1:minLengthrx);
r3x=r3x(1:minLengthrx);
r4x=r4x(1:minLengthrx);

r1y=r1y(1:minLengthry);
r3y=r3y(1:minLengthry);
r4y=r4y(1:minLengthry);

r1z=r1z(1:minLengthrz);
r3z=r3z(1:minLengthrz);
...
vx=mean([vx13 vx34 vx14]);% see the shape of the tetrahedron to check
vy=mean([vy13 vy34 vy14]);%  which pairs are reliable and which not
vz=mean([vz13 vz34 vz14]);% if too close, discard
...
minLength=min([length(r1),length(r3),length(r4)]);% normalise wrt the shortest dataset
r1=r1(1:minLength);
%r2=r2(1:minLength);
r3=r3(1:minLength);
r4=r4(1:minLength);
