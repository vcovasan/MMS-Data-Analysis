...
t1=epoch1(i1); %all in nanoseconds
t2=epoch2(i2);
t3=epoch3(i3);
t4=epoch4(i4);

t12=abs(t2-t1);% crossing time in nanoseconds
t13=abs(t3-t1);
t14=abs(t4-t1);
t23=abs(t3-t2);
t24=abs(t4-t2);
...
r_gse1 = spdfcdfread('mms1.cdf', 'Variables', 'mms1_fgm_r_gse_srvy_l2'); %position vectors
...
r1y = r_gse1(:,2);
r1z = r_gse1(:,3);
r1x = interp1(eState1, r1x, epoch1);% linear interpolation of the spaccecraft position 
...
r2y = r_gse2(:,2);
r2z = r_gse2(:,3);
r2x = interp1(eState2, r2x, epoch2);
r2y = interp1(eState2, r2y, epoch2);
r2z = interp1(eState2, r2z, epoch2);
...
r3y = r_gse3(:,2);
r3z = r_gse3(:,3);
r3x = interp1(eState3, r3x, epoch3);
r3y = interp1(eState3, r3y, epoch3);
...
minLengthX=min([length(r1x) length(r2x) length(r3x) length(r4x)]);
minLengthY=min([length(r1y) length(r2y) length(r3y) length(r4y)]);
minLengthZ=min([length(r1z) length(r2z) length(r3z) length(r4z)]);

r1x=r1x(1:minLengthX);
r1y=r1y(1:minLengthY);
r1z=r1z(1:minLengthZ);

r2x=r2x(1:minLengthX);
r2y=r2y(1:minLengthY);
r2z=r2z(1:minLengthZ);

r3x=r3x(1:minLengthX);
r3y=r3y(1:minLengthY);
r3z=r3z(1:minLengthZ);

r4x=r4x(1:minLengthX);
r4y=r4y(1:minLengthY);
r4z=r4z(1:minLengthZ);
...
[sep12 sep13 sep14 sep23 sep24 sep34]

sprintf('dot products of the separations with the normal to shock:')
sepVec12=dot([sx12(i1) sy12(i1) sz12(i1)],N)/norm([sx12(i1) sy12(i1) sz12(i1)]);%if any of these approaches 0 disrecard that pair
sepVec13=dot([sx13(i1) sy13(i1) sz13(i1)],N)/norm([sx13(i1) sy13(i1) sz13(i1)]);
sepVec14=dot([sx14(i1) sy14(i1) sz14(i1)],N)/norm([sx14(i1) sy14(i1) sz14(i1)]);
sepVec23=dot([sx23(i2) sy23(i2) sz23(i2)],N)/norm([sx23(i2) sy23(i2) sz23(i2)]);
sepVec24=dot([sx24(i2) sy24(i2) sz24(i2)],N)/norm([sx24(i2) sy24(i2) sz24(i2)]);
sepVec34=dot([sx34(i3) sy34(i3) sz34(i3)],N)/norm([sx34(i3) sy34(i3) sz34(i3)]);

[sepVec12 sepVec13 sepVec14 sepVec23 sepVec24 sepVec34]


vx12=sx12/t12;% in km/nanosecond
vx13=sx13/t13;
vx14=sx14/t14;
vx23=sx23/t23;
vx24=sx24/t24;
...
vx=mean([vx12 vx13 vx14 vx23 vx24 vx34]);% see the shape of the tetrahedron to check
vy=mean([vy12 vy13 vy14 vy23 vy24 vy34]);%  which pairs are reliable and which not
vz=mean([vz12 vz13 vz14 vz23 vz24 vz34]);% if too close, discard 
...
r_gse1=r_gse1(:,4); % all in km/nanosecond
r_gse2=r_gse2(:,4);
r_gse3=r_gse3(:,4);
r_gse4=r_gse4(:,4);
...
minLength=min([length(r1),length(r2),length(r3),length(r4)]);% normalise wrt the shortest dataset 
r1=r1(1:minLength);
r2=r2(1:minLength);
r3=r3(1:minLength);
r4=r4(1:minLength);
...
sprintf('Individual velocities:')
Vi = [Vs12 Vs13 Vs14 Vs23 Vs24 Vs34]
vmin=min([Vs12 Vs13 Vs14 Vs23 Vs24 Vs34]);
Vi./vmin

sprintf('Average shock velocity:')
Vshock = mean(Vi)
